...
export async function POST(request: NextRequest) {
  try {
    ...
    // DEMO MODE - Return mock sellback result
    // In demo mode, we'll simulate adding the buyback price to the user's balance
    const mockCurrentBalance = 80.01; // This would be from the previous box open
    const newBalance = mockCurrentBalance + (buyback_price || 0);

    return NextResponse.json({
      success: true,
      amount_credited: buyback_price || 0,
